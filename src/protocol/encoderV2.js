...
const SmartBuffer = require('../utils/expandableBuffer')
...
module.exports = class SmartEncoder {
  ...
  static sizeOfVarLong(value) {
    let longValue = SmartEncoder.encodeZigZag64(value)
    let bytes = 1
  ...
  static sizeOfVarIntBytes(value) {
    ...
    if (size < 0) {
      return SmartEncoder.sizeOfVarInt(-1)
    }

    return SmartEncoder.sizeOfVarInt(size) + size
  }
  ...
  writeEncoder(value) {
    if (value instanceof SmartEncoder !== true) {
  ...
  writeEncoderArray(value) {
    if (!Array.isArray(value) || value.some(v => !(v instanceof SmartEncoder))) {
  ...
  writeVarInt(value) {
    const byteArray = []
    let encodedValue = SmartEncoder.encodeZigZag(value)
